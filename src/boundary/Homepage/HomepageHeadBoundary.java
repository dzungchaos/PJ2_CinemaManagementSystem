...
public class HomepageHeadBoundary {
    @FXML
    ...
    @FXML
    public Button buttonHeadTransfer;
    ...
    @FXML
    public void doTransferHead(ActionEvent event) throws IOException {
        FXMLLoader loader = new FXMLLoader();
        loader.setLocation(getClass().getResource("/boundary/ManagerHeadTransfer/HeadTransferBoundary.fxml"));
        Parent parent = loader.load();
        Stage stage = new Stage(StageStyle.DECORATED);
        stage.setTitle("CHUYỂN NHƯỢNG QUYỀN CHỦ RẠP");
        stage.setScene(new Scene(parent));
        HeadTransferBoundary boundary = loader.getController();
        boundary.initData(currentUser);
        stage.initOwner((Stage) buttonHeadTransfer.getScene().getWindow());
        stage.initModality(Modality.WINDOW_MODAL);
        stage.showAndWait();
        users.clearData();
        users.loadUsers();
